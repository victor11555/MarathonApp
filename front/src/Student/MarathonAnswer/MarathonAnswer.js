            ...
            <div style={{ position: 'relative' }}>
                ...
                <div style={{ margin: '50px' }}>
                    ...
                    <div className='d-flex align-items-center justify-content-center'>
                        ...
                        <Button style={{ margin: '0 25px' }} className='btn font-weight-bolder' variant="danger" onClick={(e) => {
                            e.preventDefault();
                ...
                <ListGroup className='d-flex align-items-center justify-content-center' variant="flush">
                    <div style={{ width: '80vh' }} className='d-flex align-items-center justify-content-center flex-column'>
                        <ul>
                            {marathon.tasks.map((el) => {
                                if (!checker.checkCanPass(marathon._id, el.day)) {
                                    if (el.day - 1 === 0) return (
                                        <ListGroup.Item style={{ margin: '10px 0' }}>Tasks of the {el.day} day will be piblished
                                in {marathon.timeResponse} </ListGroup.Item>
                                    ...
                                    return (
                                        <div className={` ${style.rgba}`}>
                                            <li style={{ padding: '7px' }} className='d-flex align-items-center justify-content-center'>
                                                ...
                                                <ul>
                                                    {el.task.map((task, i) => {
                                                        if (!checker.checkAnswer(marathon._id, user._id, el.day, i)) {
                                                            return (
                                                                <li>
                                                                    <div style={{ margin: '3px' }} className='fw-normal'>Task {i + 1}</div>
                                                                    <div style={{ margin: '3px' }}>Description: {task.description}</div>
                                                                    {state === i ? null : <form onSubmit={e => submitHandler(e, el.day, i + 1, task._id)}>
                                                                        <input placeholder={'Your solution'} />
                                                                        <button style={{ margin: '0 15px' }} className='btn btn-outline-danger' type={'submit'}>Send</button>
                                                                    </form>}
                                                                </li>
                                                            )
