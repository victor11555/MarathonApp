...
export default function CompanySignUpForm() {
  ...
  const submitHandler = (e) => {
    ...
    const tlg = e.target.children[4].children[1].value;

    fetch(signUpURL, {
      method: 'POST',
      headers: {'Content-type':'Application/json'},
      body: JSON.stringify({username, company, email, password, tlg, role: 'company'})
    })
      .then(res => res.json())
      .then(response => {
  ...
  return (
    <Form onSubmit={submitHandler}>
      ...
      <Form.Group controlId="formBasicTelegram">
        ...
        <Form.Control type="string" placeholder="Add your telegram" required/>
      </Form.Group>
